...
// define the router after api/guide/auth/

// Router 1: this is for creating the new guide
router.post(
	"/createuser",
	...
	async (req, res) => {
		...
		try {
			...
			res.status(200).json(new Response(200, "guide created", { authtoken }));
...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			const authtoken = jwt.sign(data, JWT_SECREAT);
			res.status(200).json(new Response(200, "guide logged in", { authtoken }));

			logger.info("guide logged in");
		} catch (error) {
			logger.error(error);
			res.status(500).json(new Response(500, "Internal server error", error.message));
		}

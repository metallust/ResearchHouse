...
// define the router after api/committee/auth/

// Router 1: this is for creating the new committee
router.post(
	"/createuser",
	...
	async (req, res) => {
		...
		try {
			...
			res.status(200).json(new Response(200, "committee created", { authtoken }));
...
router.post(
	...
	async (req, res) => {
		...
		try {
			...
			const authtoken = jwt.sign(data, JWT_SECREAT);
			res.status(200).json(new Response(200, "committee logged in", { authtoken }));
